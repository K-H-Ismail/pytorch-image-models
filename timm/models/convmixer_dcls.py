...
class ConvMixer(nn.Module):
    def __init__(
        ...
        self.kernel_count = kwargs.get('dcls_kernel_count', int((depth / (depth + 2))  * kernel_size**2))
        self.dilated_kernel_size = kwargs.get('dcls_kernel_size', 25)
        self.dcls_sync = kwargs.get('dcls_sync', False)
        ...
        self.blocks = nn.Sequential(
            *[nn.Sequential(
                    Residual(nn.Sequential(
                        cDcls2d(dim, dim, kernel_count=self.kernel_count, dilated_kernel_size=self.dilated_kernel_size,
                            groups=dim, padding=(self.dilated_kernel_size - 1) // 2),
                        act_layer(),
                        nn.BatchNorm2d(dim)
                    )),
        ...
        if self.dcls_sync:
            P = torch.Tensor(2, dim, 1, self.kernel_count)
            with torch.no_grad():
                lim = (self.dilated_kernel_size // 2)
                torch.nn.init.normal_(P, 0, 0.5).clamp_(-lim,lim)
                P = torch.nn.parameter.Parameter(P.detach().clone())
            for i in range(depth):
                self.blocks[i][0].fn[0].P = P
    ...
    def clamp_parameters(self):
        with torch.no_grad():
            lim = self.dilated_kernel_size // 2
            for i in range(4):
                self.blocks[i][0].fn[0].P.clamp_(-lim, lim)
...
@register_model
def convmixer_dcls_1536_20(pretrained=False, **kwargs):
    model_args = dict(dim=1536, depth=20, kernel_size=9, patch_size=7, **kwargs)
    return _create_convmixer('convmixer_dcls_1536_20', pretrained, **model_args)


@register_model
def convmixer_dcls_768_32(pretrained=False, **kwargs):
    model_args = dict(dim=768, depth=32, kernel_size=7, patch_size=7, act_layer=nn.ReLU, **kwargs)
    return _create_convmixer('convmixer_dcls_768_32', pretrained, **model_args)


@register_model
def convmixer_dcls_1024_20_ks9_p14(pretrained=False, **kwargs):
    model_args = dict(dim=1024, depth=20, kernel_size=9, patch_size=14, **kwargs)
    return _create_convmixer('convmixer_dcls_1024_20_ks9_p14', pretrained, **model_args)
